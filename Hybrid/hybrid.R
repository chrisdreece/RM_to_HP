...
assignments <- assignments %>%
  ...
  select(PositionKey,X9.1.25:X3.1.27)
...
# 2) Simple monthly rollup with starting/ending + category nets
demand_swa <- assignments %>%
  ### OPTIONAL FILTER by Tier1 if use does not choose 'overall'
  ### filter(Assignment.Tier1DirectIndirect==[USER SELECTION]) %>%
  group_by(IntervalDate) %>%
  summarise(RawSWADemand=round(sum(Metric)),RoundedSWADemand=sum(Demand)) %>%
  ungroup()

summary_by_date_simple <- summary_by_date_simple %>%
  left_join(.,demand_swa,by='IntervalDate') %>%
  mutate(TotalDemand=EndingHeadcount+RoundedSWADemand) %>%
  select(IntervalDate,StartingHeadcount,EndingHeadcount_Supply=EndingHeadcount,EndingHeadcount_SWA=RoundedSWADemand,TotalDemand)
